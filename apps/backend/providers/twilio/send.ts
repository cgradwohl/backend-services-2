...
const send: DeliveryHandler = async (params, templates) => {
  ...
  if (!config.accountSid || !config.accountSid.length) {
    ...
    !config.messagingServiceSid.length
  ) {
    throw new ProviderConfigurationError("No Messaging Service SID specified.");
  }
  ...
  let body: any = {
    ...
    MessagingServiceSid:
      override?.config?.messagingServiceSid ??
      override?.config?.MessagingServiceSid ??
      config.messagingServiceSid,
  ...
  try {
    const response = await axios.post(url, data, {
      ...
      timeoutErrorMessage: "Twilio API request timed out.",
    });
