// TODO:  make body type SqsRouteMessage import { SqsRouteMessage } from "~/types.internal";

export default (body: any) => ({
  Records: [
    {
      messageId: "70aab895-346d-4414-bbda-e352cfeffc29",
      receiptHandle:
        "",
      body: JSON.stringify({ ...body, type: "route" }),
      attributes: {
        ApproximateReceiveCount: "1",
        SentTimestamp: "1578067190993",
        SenderId: "AROAZEVQJGIN722LDCLYQ:backend-dev-ApiSend",
        ApproximateFirstReceiveTimestamp: "1578067191001",
      },
      messageAttributes: {},
      md5OfBody: "87464d97c49b6b676e41a40bc07b425c",
      eventSource: "aws:sqs",
      eventSourceARN:
        "arn:aws:sqs:us-east-1:628508668443:dev_backend-sqs-route",
      awsRegion: "us-east-1",
    },
  ],
});
