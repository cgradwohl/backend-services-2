...
describe("plain template blocks", () => {
  const testBlocks = [
    "textBlock",
    "actionBlock",
    "actionBlockLink",
  ...
  testBlocks.forEach((block) => {
    ...
    fn(`should render ${block}`, () => {
      ...
      const plainTemplate = template.plainRenderer({ defaultText: "" });
      const data = {
        data: mockData,
        profile: mockProfile,
        urls: { opened: "https://abc.ct0.app/o/open-tracking" },
      };
      const rendered = plainTemplate.render(
...
describe("plain slate", () => {
  ...
  testSlate.forEach((templateName) => {
    ...
    fn(`should render ${templateName}`, () => {
      ...
      const plainTemplate = template.plainRenderer();
      const data = {
        data: mockData,
        profile: mockProfile,
        urls: { opened: "https://abc.ct0.app/o/open-tracking" },
